...
- numerical summaries - elements of `dplyr`
...
Select specific columns/variables of a data frame (vertical selection):
...
select a subset of the rows/observations (horizontal selection):
...
```{r}
head(mutate(fbi, Rate = Count/Population*66000))
```
...
i.e. the output of one function is used as the first argument in the next function. 
...
```{r}
summarize(fbi, 
  mean_rate = mean(Count/Population*66000, na.rm=TRUE), 
  sd_rate = sd(Count/Population*66000, na.rm = TRUE))
```
...
fbi2017 %>%
    group_by(Type) %>%
    summarize(mean_rate = mean(Count/Population*66000, na.rm=TRUE), 
              sd_rate = sd(Count/Population*66000, na.rm = TRUE))
```
