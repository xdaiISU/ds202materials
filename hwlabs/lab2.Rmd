...
- You can finish this lab either individually or (optionally) in a team of two students.
- Create a repository (one per team) & give access to your teammate (if you team up)
...
By now, over 2400 different chocolate bars have been rated. 
...
The `choco` dataset in the `classdata` library is a web scrap from the chocolate flavor data base.
...
Also check out `?choco` for the help documentation. If you don't have `classdata` installed, install it using `devtools::install_github('xdaiISU/classdata')`
...
## Workflow if You Work Individually

1. Log into github, and create a repository called `ds202_lab2`
3. Push `individual1.Rmd` and `individual1.html` to the remote. Review the steps to push to GitHub used in Lab 1 if you need to.


## Workflow if You Work with a Teammate

### Overview
1. Discuss with your teammate plans for the analyses.
1. Include your own analysis in an individual file. This file is the "scratch file", which should include all your own exploration and analysis, even if you would not show those results in your final report. Name your own file as either `individual1.Rmd` or `individual2.Rmd` after coordinating with your teammate. 
...
1. Check your mailbox and accept the invitation to work on the team repo.
...
- If you work in a team, then all `Rmd` and the knitted `html` files for the individuals and the team are required. 

## Repo Structure Requirement (for Everyone)
The git repo should be named ds202_lab2. Use the same folder structure and file names as follows. Files in parentheses are needed only if you work with a partner.
