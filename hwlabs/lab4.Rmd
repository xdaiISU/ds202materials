...
## Background
An excerpt of the individual player data of the Cyclone football team is extracted from the ISU Athletics website and is available here as an Excel file: [2020 data file](../slides/cyclonesFootball2020.xlsx).
...
2. All offensive and defensive statistics are numerical. The `Weight` column in biogrpahy is also numeric. (Optional: Learn to mutate multiple columns simultaneously using the `?across` function in `dplyr`)
...
2. Comment on the distributions of the offensive statistics. Explain the patterns you see.
3. Did ISU have better offense against Oregon or Oklahoma? Answer this question by creating a scatterplot with x- and y-axes being the receiving yards (for each player). A large number of receiving yards indicates good offense. 
...
1. Find which states do the ISU players (both defensive and offensive) come from. Create an appropriate summary.
...
- If you team up, agree on a method to clean the dataset with your teammate. Teammates can then work on the same cleaned version for downstream analysis.
